...
import kotlinx.android.synthetic.main.history_inventory_ceo.*
import kotlinx.android.synthetic.main.historypengajuan_barang_hrd.*

class HistoryInventoryBarangFragmentCeo: Fragment(),
    ...
    override fun onCreateView(
        ...
        return localInflater.inflate(
            R.layout.history_inventory_ceo,
            container,
            false
        )
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        rvListinventoryceo?.layoutManager = LinearLayoutManager(requireContext())
        progressBarinventoryceo?.visibility = View.VISIBLE
        ...
        swipeRefreshLayout = view.findViewById(R.id.swipehistoryinventoryceo)
    ...
    override fun onGoodsListFile(result: BrgStatus<MFile>) {
        progressBarinventoryceo?.visibility = View.GONE

        if (result.isStatus) {
            val adapter = HistoryInventoryAdapter(result.dataBarang)
            adapter.setListener { barang, _, _ ->
                val intent = Intent(context, DetailHistoryInventoryCeo::class.java)
                intent.putExtra("data", Gson().toJson(barang))
                startActivity(intent)
            }

            rvListinventoryceo?.adapter = adapter
        } else {
            Toast.makeText(
                requireContext(),
